...
struct bmimage {
    ...
    //std::shared_ptr<bm_image> bmimg;  //可以是gpu tensor
    std::string filename;
    int channel_id;
    int frame_id;
};
...
struct bm_tensor_t
{
    ...
    std::vector<std::string> imgs; // 测试时使用string
};

struct bmtensor {
    std::shared_ptr<bm_tensor_t> bmtensor;// 可以替换成自己框架内部的tensor  batch
...
private:
    ...
    std::vector<std::vector<cv::Mat>> m_resized_imgs;
